 ...
 * The null character (e.g '\0') is overwritten in the first string but added it at the end of the second one.
 *
...
/**
 * Concatenate n bytes from the second string to the first one. '\0' is overwritten in the first string. In all case, a null byte will end the first string.
 *
 * BE CARFUL, BUFFER OVERFLOW THEAT
 *
 * param:   char *s1, first string
...
/**
 * Copy the string pointed to by src (null byte included) to dest. dest must belarge enough.
 * BE CARFUL, BUFFER OVERFLOW THEAT
 *
...
/**
 * Copy almost n bytes of the string pointed to by src to dest. dest must be large enough. 
 * Warning!: If their is no null byte in the n copied bytes of src, the string will be not null-terminated.
 * BE CARFUL, BUFFER OVERFLOW THEAT
 *
 * param:   char *dest, first string
 *          char *src, second string
...
/**
 * Compare two string. It returns an integer less than, equal to, or greater than zero if s1 is found, respectively, to be less than, to match, or be greater than s2.
 *
 *
...
/**
 * The same that my_strcmp, but it compares almost n first bytes of the two string s1 and s2.
 *
 * param:   char *s1, char *s2
 * return:  int result
 **/
